export const QUALITY_NAMES = new Map([
  [127, "Highest"], [126, "DolbyVision"], [125, "HDR"], [120, "4K"],
...
export function videoTagsUrl(page) {
  if (page.bvid) return `https://api.bilibili.com/x/tag/archive/tags?bvid=${encodeURIComponent(page.bvid)}`;
  if (page.aid) return `https://api.bilibili.com/x/tag/archive/tags?aid=${page.aid}`;
  return "";
}
...
function qualityLabel(playData, qualityId) {
  ...
  return QUALITY_NAMES.get(Number(qualityId)) || playData.accept_description?.[index] || `Q${qualityId}`;
}

export function selectMedia(playPayload, requestedQuality, baseName) {
  if (Number(playPayload?.code) !== 0) throw new Error(playPayload?.message || `Playback API error ${playPayload?.code}`);
  const data = playPayload.result || playPayload.data;
  if (!data) throw new Error("The playback API returned no data");
  if (data.is_drm || data.drm_tech_type || data.drmTechType) throw new Error("This resource uses DRM and will not be downloaded");
  ...
  if (Array.isArray(data.durl) && data.durl.length) {
    ...
    return {
      ...
      media: {
        ...
        audioCodec: "aac",
        audioChannels: 2,
        qualityCode: Number(data.quality || requestedQuality || 0),
        qualityLabel: label,
  ...
  if (!videos.length) throw new Error("No downloadable non-DRM video stream is available");
  ...
  if (!videoUrl) throw new Error("The video stream URL is empty");
  ...
  return {
    ...
    media: {
      ...
      audioCodec: "aac",
      audioChannels: 2,
      qualityCode: Number(selectedId || 0),
      qualityLabel: label,
      format: data.format || "dash",
      videoCodecid: Number(video.codecid || 0),
      videoBandwidth: Number(video.bandwidth || 0),
      frameRate: video.frameRate || video.frame_rate || "",
      audioId: Number(audio?.id || 0),
      audioBandwidth: Number(audio?.bandwidth || 0),
    },
  };
}
