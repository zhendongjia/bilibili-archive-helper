...
export function selectMedia(playPayload, requestedQuality, baseName) {
  ...
  const videoUrl = bestUrl(video);
  const audioUrl = bestUrl(audio);
  if (!videoUrl) throw new Error("视频流地址为空");
  const label = qualityLabel(data, selectedId);
  const codec = String(video.codecs || "").split(".")[0] || "video";
  const items = [{ kind: "url", url: videoUrl, filename: `${baseName}_${label}_${codec}_video.m4s` }];
  if (audioUrl) items.push({ kind: "url", url: audioUrl, filename: `${baseName}_${label}_audio.m4s` });
  return {
    type: "dash",
    quality: selectedId,
    label,
